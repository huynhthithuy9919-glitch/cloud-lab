...
class OnFunctionTest {
    ...
    onVoidAction = OnFunc();
    onMessage = OnFunc();
    onFPCCMessage = OnFunc();
    onFPCCEvtNeedsLogin = OnFunc();
}
it("OnFunctionTest", async () => {
    ...
    const unregs = [];
    const asyncFns = new Array(3).fill(0).map(() => vi.fn((_a, _b) => sleep(10)));
    for (const fn of asyncFns) {
        unregs.push(test.onFunAction(fn));
    }
    const fns = new Array(3).fill(0).map(() => vi.fn((_a, _b) => 10));
    for (const fn of fns) {
        unregs.push(test.onFunAction(fn));
    }
...
//# sourceMappingURL=on-func.test.js.map