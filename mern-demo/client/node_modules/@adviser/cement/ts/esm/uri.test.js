...
describe("URI", () => {
    ...
    it("vibes calculate implict port", () => {
        ...
        const hostname = `${bindings.appSlug}--${bindings.userSlug}.${hostnameBase.replace(/^\./, "")}`;
        const buri = BuildURI.from(`http://template`);
        expect(buri.protocol(protocol).hostname(hostname).pathname(`~${bindings.fsId}~`).toString()).toBe(`https://${hostname}/~${bindings.fsId}~`);
    });
    it("vibes calculate http", () => {
        ...
        const hostname = `${bindings.appSlug}--${bindings.userSlug}.${hostnameBase.replace(/^\./, "")}`;
        const buri = BuildURI.from(`http://template`);
        expect(buri.protocol(protocol).hostname(hostname).pathname(`~${bindings.fsId}~`).toString()).toBe(`http://${hostname}/~${bindings.fsId}~`);
    });
