...
class OnlyForEach {
    ...
    forEach(callback, thisArg) {
        for (const [k, v] of Object.entries(this._my)) {
            if (Array.isArray(v)) {
                for (const vv of v) {
                    callback.call((thisArg ?? this), vv, k, this);
                }
            }
            else {
                callback.call((thisArg ?? this), v, k, this);
            }
        }
    }
}
describe("HttpHeader", () => {
    ...
    it("Merge two HttpHeaders", () => {
        ...
        const headers = HttpHeader.from(Object.fromEntries(Object.entries({
            ...CORS,
        }).map(([k, v]) => [k.toUpperCase(), v])), CORS).AsRecordStringString();
        expect(headers).toEqual(Object.fromEntries(Object.entries(CORS).map(([k, v]) => [k.toLowerCase(), v])));
    });
    it("OnlyForEach Headers", () => {
        const h = HttpHeader.from(new OnlyForEach({
            "Content-Type": "application/json",
            Array: ["a", "b"],
