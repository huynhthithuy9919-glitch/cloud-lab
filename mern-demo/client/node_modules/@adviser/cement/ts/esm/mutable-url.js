...
export function* URLSearchParamsEntries(src) {
    const entries = [];
    src.forEach((v, k) => {
        entries.push([k, v]);
    });
...
export class ReadonlyURL extends URL {
    ...
    _lazy;
    _init;
    static fromThrow = (urlStr, lazy = false) => {
    ...
    get host() {
        ...
        if (!this._hasHostpart) {
            throw new Error(`you can use host only if protocol is ${JSON.stringify(this._init)} ${JSON.stringify(Array.from(hasHostPartProtocols.keys()))}`);
        }
    ...
    get hostname() {
        ...
        if (!this._hasHostpart) {
            throw new Error(`you can use hostname only if protocol is ${JSON.stringify(this._init)} ${JSON.stringify(Array.from(hasHostPartProtocols.keys()))}`);
        }
    ...
    get port() {
        ...
        if (!this._hasHostpart) {
            throw new Error(`you can use port only if protocol is ${JSON.stringify(this._init)} ${JSON.stringify(Array.from(hasHostPartProtocols.keys()))}`);
        }
