...
(0, vitest_1.describe)("Evento", () => {
    ...
    (0, vitest_1.it)("exposes ctx id", async () => {
        const evo = new evento_js_1.Evento(reqRes);
        const send = new TestSend();
        evo.push({
            hash: "regular-handler-0",
        ...
        for (const checkId of [send.fn.mock.calls[0][1], send.fn.mock.calls[2][1], send.fn.mock.calls[3][1]]) {
            (0, vitest_1.expect)(checkId).toBe(ret.Ok().id);
        }
    });
    (0, vitest_1.it)("use ctx id", async () => {
        const evo = new evento_js_1.Evento(reqRes);
        const send = new TestSend();
        evo.push({
            hash: "regular-handler-0",
        ...
        (0, vitest_1.expect)(ret.Ok().id).toBe("test-ctx-id-123");
        (0, vitest_1.expect)(send.fn.mock.calls.map((i) => i[0])).toEqual([
            "regular-handler-0-validate",
            "start",
            "regular-handler-0-handle",
        ...
        for (const checkId of [send.fn.mock.calls[0][1], send.fn.mock.calls[2][1], send.fn.mock.calls[3][1]]) {
            (0, vitest_1.expect)(checkId).toBe("test-ctx-id-123");
        }
    });
});
