...
function disableVerbose(fn) {
    const verbose = zx_1.$.verbose;
    zx_1.$.verbose = false;
    return fn().finally(() => {
        zx_1.$.verbose = verbose;
    });
}
async function preparePubdir(pubdir, version, baseDir, srcDir) {
    ...
    if (cjsFiles.length > 0) {
        await (0, zx_1.$) `pnpm exec jscodeshift -s --parser=babel -t=./to-cjs.js ${cjsFiles}`;
    }
    ...
    await (0, zx_1.$) `cp ./tsconfig.json ./${pubdir}/`;
    await (0, zx_1.$) `cp ./deno.json ./${pubdir}/`;
    await (0, patch_version_cmd_js_1.patchVersionCmd)().handler({ version, files: [`${pubdir}/package.json`, `${pubdir}/deno.json`] });
    await (0, setup_deno_json_cmd_js_1.setUpDenoJsonCmd)().handler({ packageJson: `${pubdir}/package.json`, jsrJson: `${pubdir}/deno.json` });
    (0, zx_1.cd)(pubdir);
    await (0, zx_1.$) `pnpm pack 2>&1 | head -10 && echo "..."`;
    await (0, zx_1.$) `deno publish --dry-run --unstable-sloppy-imports --allow-dirty --allow-slow-types --quiet`;
    (0, zx_1.cd)(rootDir);
