...
async function interPoller(fn, options, stats, abortController) {
    do {
        try {
            stats.attempts += 1;
            const abortCheck = new future_js_1.Future();
            function onAbort() {
                abortCheck.resolve({
                    state: "aborted",
            ...
            if (options.actionTimeoutMs > 0) {
                races.push((0, promise_sleep_js_1.sleep)(options.actionTimeoutMs, fnAbortController.signal).then((res) => {
                ...
                case "waiting":
                    {
                        ...
                        const res = await (0, promise_sleep_js_1.sleep)(options.intervalMs, abortController.signal);
                        switch (true) {
                            case res.isAborted:
                                return {
                                    state: "aborted",
...
async function poller(fn, ioptions = {}) {
    const options = {
        intervalMs: 1000,
        timeoutMs: 30000,
        actionTimeoutMs: -1,
    ...
    if (options.timeoutMs > 0) {
        races.push((0, promise_sleep_js_1.sleep)(options.timeoutMs, abortController.signal)
