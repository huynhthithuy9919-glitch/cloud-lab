...
it("waitFor preset value", async () => {
    const wfv = new WaitingForValue({ presetValue: Option.Some(7) });
    const size = 10;
    const promiseWaits = Promise.all(Array(size)
        .fill(0)
        .map(() => wfv.waitValue()));
...
it.skip("waitfor throw", async () => {
    ...
    expect(results.mock.calls).toEqual([
        [
            Array(size).fill(expect.objectContaining({
                status: "rejected",
