import { LRUMap, AppContext } from "@adviser/cement";
...
it("test entries iterator", () => {
    ...
    expect(entries.next().value.slice(0, -1)).toEqual(["a", 1]);
    expect(entries.next().value.slice(0, -1)).toEqual(["b", 2]);
...
it("onAdd", () => {
    ...
    ufn2();
    cache.set("d", 1);
    expect(fn1.mock.calls.map(([a, b, c]) => [a, b, c.update])).toEqual([
        ["a", 1, false],
        ["a", 3, true],
        ["b", 2, false],
    ]);
    expect(fn2.mock.calls.map(([a, b, c]) => [a, b, c.update])).toEqual([
        ["a", 1, false],
        ["a", 3, true],
        ["b", 2, false],
        ["a", 2, true],
...
it("onDelete", () => {
    ...
    cache.set("b", 2);
    cache.clear();
    expect(fn1.mock.calls.map(([a, b, c]) => [a, b, c.update])).toEqual([["a", 3, true]]);
    expect(fn2.mock.calls.map(([a, b, c]) => [a, b, c.update])).toEqual([
        ["a", 3, true],
        ["b", 2, true],
    ]);
    expect(fn3.mock.calls.map(([a, b, c]) => [a, b, c.update])).toEqual([
        ["a", 1, true],
        ["b", 2, true],
    ]);
});
...
//# sourceMappingURL=lru-map-set.test.js.map