...
it("rechunk smaller 10 pack smaller chunks", async () => {
    const chunks = await rebufferArray(Array(7)
        .fill(0)
        .map((_, i) => {
...
it("rechunk smaller 10 pack bigger chunks", async () => {
    const chunks = await rebufferArray(Array(3)
        .fill(0)
        .map((_, i) => {
...
describe("test streaming through rebuffer", () => {
    ...
    it("does rebuffer respect backpressure", async () => {
        ...
        expect(state.CollectorFn).toHaveBeenCalledTimes(~~((state.sendChunkSize * state.sendChunks) / reBufferSize) + 1 + 1);
