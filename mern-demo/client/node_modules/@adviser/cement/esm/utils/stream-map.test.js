...
it("stream2array", async () => {
    const as = await stream2array(new ReadableStream({
        start(controller) {
...
it("devnull", async () => {
    const cnt = await devnull(streamMap(array2stream([1, 2, 3]), {
...
it("stream_map", async () => {
    ...
    const s = await stream2array(streamMap(array2stream([1, 2, 3]), {
        Map: (i, idx) => (idx + 1) * 10 + i + 1,
        Close: closeFn,
    }));
...
it("map types", async () => {
    const oo = await stream2array(streamMap(array2stream([1, 2, 3]), {
        Map: (chunk, idx) => {
            return "[" + chunk + "/" + idx + "]";
        },
    }));
...
describe("test streaming through streamMap", () => {
    ...
    it("does streamMap respect backpressure", async () => {
        ...
        expect(state.CollectorFn).toHaveBeenCalledTimes(state.sendChunks + 1);
...
describe("coerceStreamUint8", () => {
    it("default: encodes string chunks to Uint8Array", async () => {
        const result = await stream2array(coerceStreamUint8(array2stream(["Hello", " ", "World"])));
    ...
    it("default: passes Uint8Array chunks through unchanged", async () => {
        ...
        const result = await stream2array(coerceStreamUint8(array2stream(chunks)));
        expect(result).toEqual(chunks);
    });
    it("default: handles mixed string and Uint8Array chunks", async () => {
        const stream = array2stream(["Hello", new TextEncoder().encode(" World")]);
        const result = await stream2array(coerceStreamUint8(stream));
        expect(new TextDecoder().decode(new Uint8Array(result.flatMap((c) => [...c])))).toBe("Hello World");
    });
    it("custom encoder: is called for string chunks", async () => {
        const encodeSpy = vitest.fn((x) => {
            return new TextEncoder().encode(x);
        });
        await stream2array(coerceStreamUint8(array2stream(["a", "b", new Uint8Array([99])]), encodeSpy));
...
describe("coerceStreamString", () => {
    ...
    it("custom decoder: is called for Uint8Array chunks", async () => {
        ...
        const enc = new TextEncoder();
        await stream2array(coerceStreamString(array2stream([enc.encode("a"), enc.encode("b"), "c"]), decodeSpy));
