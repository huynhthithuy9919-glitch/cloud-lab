import React from "react";
import { switchColors, ovalColors } from "./VibesSwitch.styles.js";
const TEASE_PHRASE = [
    [0.5, 340],
    [0, 300],
    [0.5, 340],
    [0, 480],
    [1, 1500],
    [0, 1000],
];
export function VibesSwitch({ size = 24, className, style, isActive, onToggle, isTwinkling = false, isTeasing = false, ovalColor = ovalColors.neutral, ovalStatusKey, }) {
    const [internalActive, setInternalActive] = React.useState(true);
    const [teaseLevel, setTeaseLevel] = React.useState(0);
    const [teaseDismissed, setTeaseDismissed] = React.useState(false);
    const teasing = isTeasing && !teaseDismissed;
    React.useEffect(() => {
        if (teasing === false)
            return;
        if (typeof window.matchMedia === "function" && window.matchMedia("(prefers-reduced-motion: reduce)").matches)
            return;
        let step = 0;
        let timer;
        const advance = () => {
            const [level, dwell] = TEASE_PHRASE[step % TEASE_PHRASE.length];
            setTeaseLevel(level);
            step += 1;
            timer = setTimeout(advance, dwell);
        };
        timer = setTimeout(advance, 0);
        return () => {
            clearTimeout(timer);
            setTeaseLevel(0);
        };
    }, [teasing]);
    const showOval = ovalColor !== ovalColors.neutral;
    const ovalTraceKey = `${ovalColor}|${ovalStatusKey ?? ""}`;
    const logicalActive = isActive !== undefined ? isActive : internalActive;
    const prevLogicalActive = React.useRef(logicalActive);
    React.useEffect(() => {
        if (prevLogicalActive.current !== logicalActive) {
            prevLogicalActive.current = logicalActive;
            setTeaseDismissed(true);
        }
    }, [logicalActive]);
    const active = teasing ? teaseLevel >= 1 : !logicalActive;
    const handlePointerDown = (e) => {
        setTeaseDismissed(true);
        e.stopPropagation();
        e.preventDefault();
        const newState = !logicalActive;
        if (onToggle) {
            onToggle(newState);
        }
        if (isActive === undefined) {
            setInternalActive(newState);
        }
    };
    const originalD = `M426.866,285.985c-7.999-0.416-19.597-0.733-31.141-1.687  c-15.692-1.297-28.809-8.481-40.105-19.104c-12.77-12.008-20.478-26.828-22.714-44.177c-3.048-23.644,3.384-44.558,19.646-62.143  c9.174-9.92,20.248-17.25,33.444-20.363c7.786-1.837,15.944-2.399,23.973-2.828c9.988-0.535,20.023-0.666,30.021-0.371  c10.191,0.301,20.433,0.806,30.521,2.175c12.493,1.696,23.132,7.919,32.552,16.091c14.221,12.337,22.777,27.953,25.184,46.594  c2.822,21.859-2.605,41.617-16.777,58.695c-9.494,11.441-21.349,19.648-35.722,23.502c-6.656,1.785-13.724,2.278-20.647,2.77  C446.914,285.721,438.682,285.667,426.866,285.985z`;
    const stretchedD = `M165.866,285.985c-7.999-0.416-19.597-0.733-31.141-1.687  c-15.692-1.297-28.809-8.481-40.105-19.104c-12.77-12.008-20.478-26.828-22.714-44.177c-3.048-23.644,3.384-44.558,19.646-62.143  c9.174-9.92,20.248-17.25,33.444-20.363c7.786-1.837,15.944-2.399,23.973-2.828c9.988-0.535,111.023-0.666,121.021-0.371  c10.191,0.301,20.433,0.806,30.521,2.175c12.493,1.696,23.132,7.919,32.552,16.091c14.221,12.337,22.777,27.953,25.184,46.594  c2.822,21.859-2.605,41.617-16.777,58.695c-9.494,11.441-21.349,19.648-35.722,23.502c-6.656,1.785-13.724,2.278-20.647,2.77  C276.914,285.721,177.682,285.667,165.866,285.985z`;
    const budgedD = `M296.366,285.985c-7.999-0.416-19.597-0.733-31.141-1.687  c-15.692-1.297-28.809-8.481-40.105-19.104c-12.77-12.008-20.478-26.828-22.714-44.177c-3.048-23.644,3.384-44.558,19.646-62.143  c9.174-9.92,20.248-17.25,33.444-20.363c7.786-1.837,15.944-2.399,23.973-2.828c9.988-0.535,65.523-0.666,75.521-0.371  c10.191,0.301,20.433,0.806,30.521,2.175c12.493,1.696,23.132,7.919,32.552,16.091c14.221,12.337,22.777,27.953,25.184,46.594  c2.822,21.859-2.605,41.617-16.777,58.695c-9.494,11.441-21.349,19.648-35.722,23.502c-6.656,1.785-13.724,2.278-20.647,2.77  C361.914,285.721,308.182,285.667,296.366,285.985z`;
    const pillD = teasing
        ? teaseLevel >= 1
            ? stretchedD
            : teaseLevel >= 0.5
                ? budgedD
                : originalD
        : active
            ? stretchedD
            : originalD;
    const pillTransform = teasing
        ? teaseLevel >= 1
            ? "translateX(3px) "
            : teaseLevel >= 0.5
                ? "translateX(1.5px) "
                : "none"
        : active
            ? "translateX(3px) "
            : "none";
    return (React.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", height: size, viewBox: "18 58 564 282", fill: "currentColor", className: className, onPointerDown: handlePointerDown, style: { overflow: "visible", ...style } },
        React.createElement("ellipse", { key: showOval ? ovalTraceKey : "ring-idle", cx: "300", cy: "242", rx: "272", ry: "128", fill: "none", stroke: ovalColor, strokeWidth: "28", strokeLinecap: "round", pathLength: 100, pointerEvents: "none", filter: showOval ? "url(#vibes-oval-ink)" : undefined, className: showOval ? "vibes-oval-trace" : undefined, style: showOval ? undefined : { opacity: 0 } }),
        React.createElement("defs", null,
            React.createElement("filter", { id: "vibes-oval-ink", x: "-30%", y: "-45%", width: "160%", height: "190%" },
                React.createElement("feTurbulence", { type: "fractalNoise", baseFrequency: "0.012", numOctaves: "1", seed: "7", result: "ink" }),
                React.createElement("feDisplacementMap", { in: "SourceGraphic", in2: "ink", scale: "6", xChannelSelector: "R", yChannelSelector: "G" })),
            React.createElement("style", null, `
          @keyframes vibes-switch-letter-twinkle {
            0%, 100% { opacity: 1; }
            50%      { opacity: 0.25; }
          }
          /* Two twinkle sites, two meanings. The LEADING face letters (V·I·B·E·S)
             twinkle on an infinite loop while the network is busy — that's the
             inline isTwinkling animation on those paths below. The TRAILING
             wordmark (D · I · Y, black-on-white) twinkles a FINITE two passes to
             say "your click landed", and nothing else. Different paths, so the
             two never fight over the same animation property.

             The trigger is an ancestor data-vibes-ack attribute, set by the
             SSR corner logo's inline script (VibeCornerLogoSsr) — the time-zero
             paint has no framework awake to answer a click, so the whole
             acknowledgment is this rule plus one attribute flip. Staggered
             durations per letter mirror the leading letters' spread; two
             iterations puts the whole run at ~1–1.6s, then it stops on its own
             (transient feedback, never a state). */
          [data-vibes-ack] .vibes-switch-diy {
            animation: vibes-switch-letter-twinkle var(--vibes-ack-duration, 0.5s) ease-in-out 2;
          }
          .vibes-switch-diy-1 { --vibes-ack-duration: 0.5s; }
          .vibes-switch-diy-2 { --vibes-ack-duration: 0.8s; }
          .vibes-switch-diy-3 { --vibes-ack-duration: 0.6s; }
          .vibes-switch-diy-4 { --vibes-ack-duration: 0.7s; }
          .vibes-switch-diy-5 { --vibes-ack-duration: 0.55s; }
          @media (prefers-reduced-motion: reduce) {
            /* Opacity flicker is exactly the kind of motion this opts out of;
               the click still lands, it just doesn't announce itself. */
            [data-vibes-ack] .vibes-switch-diy { animation: none; }
          }
          /* Draw the ring on like a felt-tip marker (stroke-dashoffset sweeps
             the dash around the path), hold it, then fade the whole trace out —
             one ~7s pass, matching the old on-screen dwell but replacing the
             flash-in with a drawn stroke.

             Throughout that whole visible life the ring also CONTRACTS toward
             its own center (cx/cy never move), monotonically, bottoming out
             exactly as it vanishes: rx/ry run from 272/128 to 79.9/37.6, a
             0.29375 scale. At the header pill (size=75, viewBox height 282 →
             0.266 CSS px per unit) that is a 68.1px-tall ellipse closing down
             to 20.0 CSS px. The end size is a fixed SCALE, so the 20px figure
             is exact only at size=75 — a size=60 switch ends at 16px, size=80
             at 21.3px.

             Animating the GEOMETRY (rx/ry) and not 'transform: scale()' is the
             load-bearing choice: a scaled ring drags its stroke down with it
             and the ring visibly thins as it closes, so the doubled
             stroke-width would only read at the very first frame. With rx/ry
             the 28-unit stroke is constant on screen for the full 7s. The
             stops below are the linear-in-time scale samples at each existing
             keyframe (0.899 at 14.3%, 0.496 at 71.4%) so the contraction
             inherits each segment's own easing instead of stair-stepping.
             'pathLength=100' keeps the dash math geometry-independent, so the
             sweep is unaffected by the shrink. */
          .vibes-oval-trace {
            stroke-dasharray: 100;
            animation: vibes-oval-trace 7s forwards;
          }
          @keyframes vibes-oval-trace {
            0%    { stroke-dashoffset: 100; opacity: 1; rx: 272;    ry: 128;    animation-timing-function: cubic-bezier(0.3, 0.6, 0.25, 1); }
            14.3% { stroke-dashoffset: 0;   opacity: 1; rx: 244.53; ry: 115.07; }
            71.4% { stroke-dashoffset: 0;   opacity: 1; rx: 134.84; ry: 63.45;  animation-timing-function: ease-in; }
            100%  { stroke-dashoffset: 0;   opacity: 0; rx: 79.9;   ry: 37.6;   }
          }
          @media (prefers-reduced-motion: reduce) {
            /* No drawing motion: show the full ring, hold, then fade — the
               trace sweep is exactly what reduced-motion opts out of. The
               contraction is NOT motion-sickness motion the way a sweep is,
               and it carries the same "this is expiring" meaning, so it runs
               here too, to the identical end state. */
            .vibes-oval-trace {
              stroke-dasharray: none;
              animation: vibes-oval-fade 7s forwards;
            }
            @keyframes vibes-oval-fade {
              0%    { opacity: 1; rx: 272;    ry: 128;    }
              71.4% { opacity: 1; rx: 134.84; ry: 63.45;  }
              100%  { opacity: 0; rx: 79.9;   ry: 37.6;   }
            }
          }
        `)),
        React.createElement("g", { transform: "translate(0,32)" },
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", fill: "#000", d: "M293.353,298.09c-41.038,0-82.078,0.125-123.115-0.077  c-11.993-0.06-24.011-0.701-35.964-1.703c-15.871-1.331-29.73-7.937-41.948-17.946c-16.769-13.736-27.207-31.417-30.983-52.7  c-4.424-24.93,1.404-47.685,16.506-67.913c11.502-15.407,26.564-26.1,45.258-30.884c7.615-1.949,15.631-2.91,23.501-3.165  c20.08-0.652,40.179-0.853,60.271-0.879c69.503-0.094,139.007-0.106,208.51,0.02c14.765,0.026,29.583,0.097,44.28,1.313  c36.984,3.059,61.78,23.095,74.653,57.301c17.011,45.199-8.414,96.835-54.29,111.864c-7.919,2.595-16.165,3.721-24.434,3.871  c-25.614,0.467-51.234,0.742-76.853,0.867C350.282,298.197,321.817,298.09,293.353,298.09z" }),
            React.createElement("path", { fill: switchColors.secondary, fillRule: "evenodd", clipRule: "evenodd", d: pillD, style: {
                    transition: "d 0.3s ease, transform 0.8s ease, fill 2s ease",
                    transform: pillTransform,
                } }),
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 0.5s ease",
                    fill: active ? switchColors.primary : switchColors.secondary,
                    animation: isTwinkling ? "vibes-switch-letter-twinkle 0.5s ease-in-out infinite" : undefined,
                }, d: "M181.891,205.861c0-5.043-0.001-10.086,0-15.129  c0.001-5.046,1.679-7.539,6.606-7.695c9.292-0.294,18.653-1.051,27.888,0.707c7.614,1.449,11.523,5.954,11.902,13.446  c0.066,1.312-0.313,2.752-0.857,3.966c-1.401,3.123-1.399,6.266-0.673,9.507c0.301,1.342,0.443,2.723,0.787,4.053  c1.274,4.925-1.78,10.114-6.085,11.937c-3.111,1.318-6.561,2.327-9.909,2.497c-7.303,0.37-14.639,0.136-21.96,0.101  c-1.165-0.005-2.345-0.181-3.488-0.422c-2.657-0.56-4.162-2.962-4.197-6.801C181.854,216.639,181.891,211.25,181.891,205.861z   M204.442,192.385c-2.757,0-5.514,0-8.271,0c-3.695,0-5.151,1.669-4.712,5.403c0.369,3.14,1.05,3.735,4.225,3.737  c5.024,0.004,10.05,0.109,15.07-0.014c2.028-0.05,4.167-0.27,6.04-0.98c3.182-1.207,3.639-4.256,1.008-6.455  c-1.073-0.896-2.659-1.509-4.06-1.618C210.659,192.22,207.544,192.385,204.442,192.385z M204.334,211.104c0,0.045,0,0.091,0,0.137  c-3.101,0-6.203-0.055-9.302,0.037c-0.823,0.024-2.257,0.373-2.344,0.794c-0.447,2.154-0.959,4.444-0.639,6.563  c0.276,1.822,2.447,1.451,3.882,1.441c5.989-0.042,11.98-0.118,17.961-0.385c1.416-0.063,2.859-0.79,4.176-1.441  c1.79-0.886,1.833-2.475,1.029-4.046c-1.166-2.276-3.297-3.024-5.677-3.081C210.394,211.049,207.363,211.104,204.334,211.104z" }),
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 0.8s ease",
                    fill: active ? switchColors.primary : switchColors.secondary,
                    animation: isTwinkling ? "vibes-switch-letter-twinkle 0.8s ease-in-out infinite" : undefined,
                }, d: "M291.409,229.748c-3.621-0.394-7.838-0.587-11.94-1.379  c-3.577-0.69-6.343-2.991-8.213-6.163c-1.763-2.99-0.301-5.6,3.139-5.292c2.287,0.205,4.512,1.129,6.758,1.755  c6.281,1.751,12.643,1.892,19.053,0.951c0.667-0.098,1.31-0.416,1.941-0.686c1.502-0.644,2.55-1.682,2.581-3.415  c0.031-1.74-1.195-2.749-2.579-3.132c-2.298-0.637-4.688-1.021-7.065-1.273c-5.062-0.536-10.252-0.401-15.187-1.475  c-9.677-2.105-11.678-10.53-10.101-16.009c1.62-5.625,5.911-8.92,11.318-9.73c8.388-1.257,16.925-1.491,25.279,0.654  c3.702,0.951,6.615,3.072,7.883,6.931c0.918,2.792-0.332,4.6-3.268,4.357c-1.684-0.139-3.367-0.676-4.974-1.248  c-6.711-2.387-13.572-2.897-20.569-1.783c-1.001,0.159-2.146,0.414-2.875,1.034c-0.901,0.766-2.016,1.981-1.98,2.964  c0.041,1.128,0.995,2.733,1.991,3.206c1.81,0.857,3.925,1.279,5.948,1.441c5.152,0.41,10.356,0.296,15.479,0.905  c7.98,0.949,13.779,9.833,11.241,17.125c-1.959,5.628-6.44,8.489-12.143,9.322C299.455,229.344,295.715,229.419,291.409,229.748z" }),
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1.2s ease",
                    fill: active ? switchColors.primary : switchColors.secondary,
                    animation: isTwinkling ? "vibes-switch-letter-twinkle 1.2s ease-in-out infinite" : undefined,
                }, d: "M235.786,208.14c0-6.905-0.01-13.809,0.004-20.714  c0.007-3.474,0.948-4.428,4.415-3.758c6.62,1.279,13.232,2.651,19.759,4.331c1.7,0.438,3.404,1.896,4.515,3.341  c1.777,2.31,0.433,5.367-2.463,5.745c-1.86,0.243-3.819-0.138-5.717-0.368c-2.183-0.264-4.339-0.783-6.525-0.976  c-1.572-0.138-3.065,0.375-3.8,1.959c-0.76,1.638-0.319,3.329,0.942,4.34c1.619,1.296,3.522,2.327,5.447,3.128  c2.146,0.894,4.539,1.207,6.66,2.145c1.446,0.64,2.982,1.687,3.786,2.981c0.689,1.11,0.928,3.094,0.378,4.202  c-0.492,0.991-2.32,1.795-3.579,1.825c-2.238,0.052-4.483-0.652-6.741-0.832c-1.614-0.127-3.333-0.203-4.865,0.212  c-2.574,0.699-3.225,3.013-1.719,5.218c1.396,2.044,3.431,3.141,5.757,3.761c2.791,0.744,5.637,1.315,8.373,2.222  c3.19,1.058,4.791,3.496,4.801,6.723c0.011,3.365-1.759,5.021-5.138,4.424c-4.402-0.778-8.759-1.81-13.134-2.735  c-2.357-0.499-4.718-0.981-7.069-1.511c-3.263-0.737-4.132-1.805-4.141-5.154c-0.019-6.836-0.006-13.672-0.006-20.508  C235.747,208.141,235.766,208.14,235.786,208.14z" }),
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 0.6s ease",
                    fill: active ? switchColors.primary : switchColors.secondary,
                    animation: isTwinkling ? "vibes-switch-letter-twinkle 0.6s ease-in-out infinite" : undefined,
                }, d: "M135.138,229.842c-2.941-0.084-5.296-1.462-6.684-3.9  c-1.827-3.21-3.328-6.618-4.81-10.011c-3.55-8.128-7.021-16.291-10.486-24.455c-0.48-1.132-0.902-2.329-1.087-3.536  c-0.417-2.72,1.238-4.585,3.938-4.119c1.591,0.275,3.569,0.98,4.45,2.173c2.226,3.015,4.175,6.299,5.784,9.69  c2.208,4.654,3.898,9.552,6.032,14.244c0.628,1.379,2.009,2.416,3.045,3.609c0.892-1.159,2.042-2.201,2.63-3.498  c2.697-5.953,5.22-11.985,7.841-17.974c1.423-3.252,3.089-6.418,6.532-7.905c1.238-0.535,3.012-0.712,4.184-0.214  c0.81,0.344,1.377,2.126,1.385,3.271c0.009,1.458-0.479,2.997-1.059,4.371c-4.227,10.013-8.504,20.005-12.833,29.974  c-0.79,1.819-1.762,3.589-2.875,5.229C139.73,228.848,137.671,229.894,135.138,229.842z" }),
            React.createElement("path", { fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1.3s ease",
                    fill: active ? switchColors.primary : switchColors.secondary,
                    animation: isTwinkling ? "vibes-switch-letter-twinkle 1.3s ease-in-out infinite" : undefined,
                }, d: "M164.636,206.263c0-6.691,0.054-13.383-0.036-20.073  c-0.024-1.851,0.716-2.67,2.449-2.81c0.274-0.022,0.549-0.054,0.823-0.076c5.488-0.445,6.091,0.105,6.091,5.562  c0,12.348,0,24.695,0,37.043c0,2.887-0.354,3.405-3.222,3.618c-1.628,0.121-3.338-0.001-4.91-0.408  c-0.593-0.153-1.265-1.408-1.278-2.171c-0.096-5.584-0.034-11.172-0.022-16.759c0.002-1.308,0-2.617,0-3.926  C164.566,206.263,164.601,206.263,164.636,206.263z" }),
            React.createElement("path", { className: "vibes-switch-diy vibes-switch-diy-1", fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1s ease",
                    fill: active ? switchColors.secondary : switchColors.primary,
                }, d: "M388.313,210.147c0-6.356,0.034-12.713-0.023-19.069  c-0.015-1.61,0.359-2.472,2.19-2.346c2.887,0.198,5.809,0.045,8.671,0.398c4.396,0.542,8.019,4.294,8.144,8.904  c0.223,8.142,0.265,16.304-0.074,24.439c-0.248,5.945-4.552,9.662-10.491,9.831c-1.999,0.057-4.003-0.081-6.006-0.09  c-1.746-0.008-2.439-0.853-2.428-2.584C388.34,223.136,388.313,216.642,388.313,210.147z M393.418,210.324c-0.037,0-0.075,0-0.114,0  c0,4.55-0.038,9.101,0.015,13.65c0.031,2.688,0.926,3.439,3.56,3.239c3.273-0.248,5.493-2.511,5.534-6.04  c0.082-7.099,0.054-14.2-0.033-21.299c-0.041-3.268-1.739-5.241-4.87-6.092c-2.68-0.728-4.025,0.161-4.07,2.896  C393.364,201.226,393.418,205.775,393.418,210.324z" }),
            React.createElement("path", { className: "vibes-switch-diy vibes-switch-diy-2", fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1s ease",
                    fill: active ? switchColors.secondary : switchColors.primary,
                }, d: "M478.079,200.8c0.674-1.566,1.121-2.53,1.506-3.519  c0.673-1.73,1.252-3.5,1.981-5.205c0.315-0.737,0.766-1.654,1.407-1.961c1.094-0.523,2.388-0.63,3.598-0.912  c0.205,1.142,0.798,2.381,0.537,3.404c-0.606,2.388-1.448,4.756-2.507,6.984c-3.981,8.389-4.352,17.254-3.78,26.282  c0.091,1.438,0.031,2.899-0.105,4.335c-0.14,1.473-0.989,2.428-2.542,2.497c-1.514,0.067-2.311-0.903-2.54-2.23  c-0.232-1.348-0.394-2.754-0.277-4.108c0.94-10.972-1.116-21.38-5.626-31.375c-0.586-1.298-0.899-2.762-1.093-4.183  c-0.233-1.712,0.825-2.592,2.379-1.843c1.164,0.561,2.345,1.55,2.973,2.657c1.078,1.897,1.712,4.043,2.568,6.07  C476.918,198.547,477.37,199.361,478.079,200.8z" }),
            React.createElement("path", { className: "vibes-switch-diy vibes-switch-diy-3", fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1s ease",
                    fill: active ? switchColors.secondary : switchColors.primary,
                }, d: "M440.516,210.627c0,6.281,0.007,12.563-0.004,18.844  c-0.004,2.067-0.805,3.038-2.531,3.015c-1.877-0.025-2.365-1.136-2.359-2.876c0.046-12.631,0.019-25.263,0.029-37.895  c0.002-2.592,0.525-3.205,2.419-3.148c1.856,0.057,2.479,1.03,2.466,2.803C440.484,197.788,440.515,204.208,440.516,210.627z" }),
            React.createElement("path", { className: "vibes-switch-diy vibes-switch-diy-4", fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1s ease",
                    fill: active ? switchColors.secondary : switchColors.primary,
                }, d: "M416.875,210.721c0.068-3.305,1.849-5.306,4.727-5.309  c2.765-0.003,4.924,2.404,4.816,5.371c-0.106,2.956-2.355,5.212-5.12,5.138C418.626,215.849,416.813,213.718,416.875,210.721z" }),
            React.createElement("path", { className: "vibes-switch-diy vibes-switch-diy-5", fillRule: "evenodd", clipRule: "evenodd", style: {
                    transition: "fill 1s ease",
                    fill: active ? switchColors.secondary : switchColors.primary,
                }, d: "M449.933,210.636c0.102-3.331,1.886-5.279,4.778-5.22  c2.67,0.055,4.829,2.432,4.762,5.243c-0.073,3.021-2.404,5.36-5.242,5.261C451.606,215.829,449.84,213.657,449.933,210.636z" }))));
}
//# sourceMappingURL=VibesSwitch.js.map